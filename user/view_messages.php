...
if (isset($_GET['inquiry_id'])) {
  ...
  $sql = "DELETE FROM inquiry WHERE inquiry_id = :inquiry_id AND user_id = :user_id";
  $stmt = $conn->prepare($sql);
  $stmt->bindParam(':inquiry_id', $_GET['inquiry_id']);
  $stmt->bindParam(':user_id', $user_id);
  if ($stmt->execute()) {
    header("Location: view_messages.php?message=Message Deleted Successfully");
    exit;
  } else {
...
<div class="content-wrapper">
  <div class="content-header">
    <div class="container-fluid">
      <div class="row mb-2">
        <div class="col-sm-12">
          <h1 class="m-0">View Message History</h1>
        </div><!-- /.col -->
      </div><!-- /.row -->
    </div><!-- /.container-fluid -->
  </div>
          ...
          <div class="card">
            <div class="card-body">
              <table class="table table-hover table-striped">
                ...
                <tbody>
                  <?php
                  $stmt = $conn->prepare("SELECT * FROM inquiry");
                  $stmt->execute();
                  ...
                  foreach ($inquiries as $inquiry) {
                    ...
                    echo "<td>" . $inquiry['admin_reply'] . "</td>";
                    echo "<td>" . ($inquiry['user_reply'] ?: "No reply yet") . "</td>";

                    // Delete Button
                    echo '<td><a href="view_messages.php?inquiry_id=' . $inquiry['inquiry_id'] . '" class="btn btn-danger" onclick="return confirm(\'Are you sure you want to delete this message?\');">Delete</a></td>';
                    echo "</tr>";
                  }
        ...
        <div class="modal-body">
          <div class="row">
            ...
            <div class="col-12 mt-3">
              ...
              <p><?php echo !empty($inquiry['user_reply']) ? nl2br($inquiry['user_reply']) : "No reply yet."; ?></p>
            </div>
          </div>
        </div>
        <div class="modal-footer">
          <form class="replyForm" method="POST" id="replyFormM<?php echo $inquiry['inquiry_id'] ?>" action="javascript:void(0);">
